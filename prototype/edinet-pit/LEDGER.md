...
`process.env` を読む箇所が4つあり、流儀がばらついていた。同じ「DBのパス」が
MCPサーバでは環境変数、CLIでは引数、レイクのパスは引数のみで、
**どちらが勝つのかコードのどこにも書かれていなかった**。

- `src/config.mjs`（新規）: `process.env` を読む唯一の場所。
  **優先順位を CLI引数 > 環境変数 > 既定値 に明文化**し、テストで固定
- `MissingSecretError`: 変数名だけでなく**取得元と注入方法**（`infisical run`）を提示。
  従来の `EDINET_API_KEY is not set.` には「で、どうすれば」が無かった
- `.infisical.json`（コミット対象。秘密を含まない。`workspaceId` はプレースホルダ）
- `.env.example`: Infisicalに載せる4つと**載せないVRT系15個の両方**を列挙。
  「どの変数がどこで管理されているか」が1ファイルで分かる状態にするのが目的
- `test/config.test.mjs` 14件。合計196件緑

**既定値は config に置かなかった**

当初 config に既定値を持たせたが、**呼び出し側で欲しい既定値が違う**ことが判明した
（CLIはファイル、MCPサーバはインメモリ＋フィクスチャ）。
共有すべきは既定値ではなく**優先順位**だったので、
`loadConfig(overrides, env, defaults)` の第3引数として呼び出し側に委ねた。

**Infisicalに入れなかったもの**

- VRT側15変数 — **秘密がゼロ**（全て動作設定）。秘密でないものを秘密ストアに入れると
  本当の秘密が埋もれる
- リクエスト間隔・リトライ回数 — 運用上の必要がまだ無く、
  **公的APIへの間隔を外から0にできるつまみを作るのは危険**

**あわせて閉じた穴**: `.gitignore` に成果物の除外が無く、リポジトリ直下で
`backfill` を走らせると**SQLite DBと数百MBのデータレイクがコミット対象になっていた**。
`git check-ignore` で除外を実測確認。

**未検証**: egress制限とCLI未インストールにより、**`infisical run` の実接続は未確認**。
検証できたのは「環境変数として与えられたときに正しく解決されること」まで。
`.infisical.json` の `workspaceId` は実プロジェクト作成後に `infisical init` で埋める必要がある。
